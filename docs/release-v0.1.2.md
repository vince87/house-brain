# Checklist di rilascio v0.1.2

Questa checklist non autorizza il tag. Il tag `v0.1.2` e la release GitHub
richiedono il collaudo sul server e il consenso esplicito.

## Preparazione

- [ ] versione applicativa, pacchetto e lockfile impostati a `0.1.2`;
- [ ] Compose pubblico impostato su `ghcr.io/vince87/house-brain:0.1.2`;
...
- [ ] health check restituisce la versione `0.1.2`;
- [ ] entità con `hidden_by` assenti da ricerca, configuratore, API e MCP;
- [ ] entità non elencate assenti da API, chat, eventi e MCP;
- [ ] `visible` concede sola lettura e `include` concede lettura e controllo;
- [ ] “Non visibile” rimuove l'entità dalla policy senza scrivere esclusioni;
- [ ] il nome iniziale deriva dal `friendly_name` di Home Assistant;
- [ ] un nome modificato nella GUI è autorevole nella risoluzione;
...
- [ ] `PRAGMA integrity_check` resta `ok` dopo riavvio e backup/ripristino;
...
- [ ] tag `v0.1.2` creato sul commit approvato;
- [ ] workflow multiarch completato;
- [ ] immagini GHCR `0.1.2`, `0.1` e `latest` disponibili;
- [ ] release GitHub pubblicata con le note del changelog;
- [ ] installazione pulita verificata con l'immagine `0.1.2`.
