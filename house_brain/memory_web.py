...
LIFECYCLE_LABELS = {
    "en": {"source":"Source","confirmed":"Confirmed","expires":"Expires","never":"Never","expired":"Expired","export":"Export","import":"Import","imported":"Memories imported."},
    "it": {"source":"Provenienza","confirmed":"Confermata","expires":"Scadenza","never":"Mai","expired":"Scaduta","export":"Esporta","import":"Importa","imported":"Memorie importate."},
    "de": {"source":"Quelle","confirmed":"Bestätigt","expires":"Ablauf","never":"Nie","expired":"Abgelaufen","export":"Export","import":"Import","imported":"Erinnerungen importiert."},
    "es": {"source":"Origen","confirmed":"Confirmada","expires":"Caduca","never":"Nunca","expired":"Caducada","export":"Exportar","import":"Importar","imported":"Memorias importadas."},
    "fr": {"source":"Source","confirmed":"Confirmée","expires":"Expiration","never":"Jamais","expired":"Expirée","export":"Exporter","import":"Importer","imported":"Mémoires importées."},
    "pt": {"source":"Origem","confirmed":"Confirmada","expires":"Expira","never":"Nunca","expired":"Expirada","export":"Exportar","import":"Importar","imported":"Memórias importadas."},
    "ar": {"source":"المصدر","confirmed":"تم التأكيد","expires":"الانتهاء","never":"أبدًا","expired":"منتهية","export":"تصدير","import":"استيراد","imported":"تم استيراد الذكريات."},
    "ja": {"source":"出所","confirmed":"確認日時","expires":"期限","never":"なし","expired":"期限切れ","export":"書き出し","import":"読み込み","imported":"メモリを読み込みました。"},
    "ko": {"source":"출처","confirmed":"확인","expires":"만료","never":"없음","expired":"만료됨","export":"내보내기","import":"가져오기","imported":"메모리를 가져왔습니다."},
...
<div class="panel"><div class="toolbar"><input id="search" type="search" placeholder="__SEARCH__"><div class="tabs"><button id="activeTab" class="active">__ACTIVE__</button><button id="trashTab">__TRASH__</button></div><button id="exportButton">__EXPORT__</button><button id="importButton">__IMPORT__</button><input id="importFile" class="hidden" type="file" accept="application/json"><button id="newButton" class="primary">__NEW__</button></div><div id="status" class="status"></div></div>
<form id="editor" class="panel hidden"><div class="form-grid"><input id="key" placeholder="__KEY__" maxlength="120" required><input id="category" placeholder="__CATEGORY__" maxlength="50" value="fact" required><input id="importance" type="number" min="1" max="10" value="5" required><label>__EXPIRES__ <input id="expiresAt" type="datetime-local"></label><textarea id="value" class="wide" placeholder="__VALUE__" maxlength="2000" required></textarea></div><div class="actions"><button class="primary">__SAVE__</button><button id="cancel" type="button">__CANCEL__</button></div></form>
...
$("exportButton").onclick=()=>{const exported=items.filter(x=>!x.deleted_at).map(x=>({key:x.key,value:x.value,category:x.category,importance:x.importance,expires_at:x.expires_at})),blob=new Blob([JSON.stringify(exported,null,2)],{type:"application/json"}),link=document.createElement("a");link.href=URL.createObjectURL(blob);link.download="house-brain-memories.json";link.click();URL.revokeObjectURL(link.href)};$("importButton").onclick=()=>$("importFile").click();$("importFile").onchange=guard(async()=>{const file=$("importFile").files[0];if(!file)return;const memories=JSON.parse(await file.text());if(!Array.isArray(memories))throw new Error("Invalid memory export");const response=await api("/memory/import",{method:"POST",body:JSON.stringify(memories)}),payload=await response.json();if(!response.ok)throw new Error(payload.detail||response.statusText);await load();message(i18n.imported);$("importFile").value=""});
