...
(()=>{"use strict";const M=__MESSAGES__,KEY="house_brain_api_key";let state={version:1,default_view:null,views:[]};
...
function sync(){view.name=name.value.trim();view.id=slug(id.value||name.value);id.value=view.id;view.areas=split(areas.value);view.domains=split(domains.value).map(x=>x.toLowerCase());view.entities=split(entities.value).map(x=>x.toLowerCase());view.max_entities=Number(limit.value)||50;view.enabled=enabled.checked;view.include_linked_memories=memories.checked;refreshDefault()}
for(const x of [name,id,areas,domains,entities,limit,enabled,memories])x.addEventListener("change",sync);
preview.onclick=async()=>{sync();out.hidden=false;out.textContent="…";try{const b=await api("/admin/context-views/"+encodeURIComponent(view.id)+"/preview");out.textContent=b.selected_before_limit+" "+M.selected+", "+b.omitted_by_view_limit+" "+M.omitted}catch(e){out.textContent=M.error+e.message}};
remove.onclick=()=>{if(confirm(M.confirm_remove)){state.views.splice(index,1);if(state.default_view===view.id)state.default_view=null;render()}};
...
async function load(){const b=await api("/admin/context-views");state=b.configuration;render();$("auth").hidden=true;$("editor").hidden=false;$("logout").hidden=false}
...
$("add").onclick=()=>{let n=state.views.length+1,id="context_view_"+n;while(state.views.some(v=>v.id===id))id="context_view_"+(++n);state.views.push({id,name:"Context view "+n,enabled:true,areas:[],domains:[],entities:[],max_entities:50,include_linked_memories:true});render()};
$("defaultView").onchange=e=>state.default_view=e.target.value||null;
$("save").onclick=async()=>{for(const input of $("views").querySelectorAll("input"))input.dispatchEvent(new Event("change"));$("status").textContent="";try{const b=await api("/admin/context-views",{method:"PUT",body:JSON.stringify(state)});state=b.configuration;render();$("status").textContent=M.saved}catch(e){$("status").textContent=M.error+e.message;$("status").classList.add("error")}};
