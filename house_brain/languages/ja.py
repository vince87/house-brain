...
REJECTION_REASONS = {
    ...
    "no_target": (
        "検索で制御可能な単一のエンティティを特定できませんでした"
    ),
    "resolved_entity": (
        "提案されたエンティティがサーバーの解決結果と一致しません"
    ),
    "not_included": (
        "指定されたエンティティ ID は制御できません"
    ),
    "action": (
        "指定されたアクションはポリシーで許可されていません"
    ),
    "value": (
        "指定された値はポリシーで許可されていません"
    ),
    "parameter": (
        "指定されたパラメーターはポリシーで許可されていません"
    ),
    "invalid": (
        "生成されたコマンドは無効です"
    ),
