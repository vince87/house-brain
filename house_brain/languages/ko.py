...
MESSAGES = {
    ...
    "entity_not_found": (
        "요청한 이름과 일치하는 엔터티를 찾지 못했습니다. "
        "장치 이름을 확인하세요."
    ),
    "entity_not_controllable": (
        "요청한 엔터티가 존재하지만 제어 가능한 엔터티에 포"
        "함되어 있지 않습니다."
    ),
...
REJECTION_REASONS = {
    ...
    "kill_switch": (
        "안전 스위치가 실제 실행을 비활성화했습니다"
    ),
    "mode": (
        "요청한 모드가 승인되지 않았습니다"
    ),
    "explicit_entity": (
        "제안된 엔터티가 요청한 엔터티 ID와 일치하지 않습"
        "니다"
    ),
    "unresolved": (
        "서버가 장치 이름을 해석하지 못했습니다"
    ),
    "no_target": (
        "검색에서 제어 가능한 단일 엔터티를 찾지 못했습니다"
    ),
    "resolved_entity": (
        "제안된 엔터티가 서버 해석 결과와 일치하지 않습니다"
    ),
    "not_included": (
        "요청한 엔터티 ID는 제어할 수 없습니다"
    ),
    "action": (
        "요청한 작업이 정책에서 승인되지 않았습니다"
    ),
    "value": (
        "요청한 값이 정책에서 승인되지 않았습니다"
    ),
    "parameter": (
        "요청한 매개변수가 정책에서 승인되지 않았습니다"
    ),
    "invalid": (
        "생성된 명령이 유효하지 않습니다"
    ),
